...
class EditEmployee extends Component {
   ...
   */
    checkDuplicateRecord = async event => {
        ...
        if (value === 'Male') {
            url = 'http://192.168.10.40:3210/checkDuplicate/' + firstName + '/' + lastName + '/' + fatherName + '/1/' + dateOfBirth + '/' + panNo;
        } else {
            url = 'http://192.168.10.40:3210/checkDuplicate/' + firstName + '/' + lastName + '/' + fatherName + '/2/' + dateOfBirth + '/' + panNo;
        }
        axios.get(url)
            .then((empData) => {
    ...
    */
    ...
    updateEmployeeData = async () => {
        ...
        var url = 'http://192.168.10.40:3210/UpdateReq/' + this.props.location.state.emp_id + '/' + this.props.location.state.companyName;
        axios.post(url, {
            emp_uid: empUEN,
            emp_id: empNumber,
            emp_fname: firstName,
