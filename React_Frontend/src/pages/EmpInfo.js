...
/**
  ...
  */
  getCompaniesList() {
    var url = 'http://ec2-34-209-178-145.us-west-2.compute.amazonaws.com:3210/compList';
    axios.get(url)
        .then((compList) => {
    ...
    renderSearch = async () => {
        if (this.state.searchuid !== "" && this.state.searchuid.length > 0) {
            var url = 'http://ec2-34-209-178-145.us-west-2.compute.amazonaws.com:3210/search/' + this.state.searchuid + '/' + this.state.company_id;
            axios.get(url)
                .then((empData) => {
                    this.setState({
